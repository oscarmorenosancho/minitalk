...
/*   Updated: 2022/12/15 13:11:42 by omoreno-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	g_queue.head_mutex = 0;
	g_queue.tail_mutex = 0;
	g_queue.head = 0;
...
int	ft_push_se(t_sig_event se)
{
	...
	if (! g_queue.head_mutex)
		g_queue.head_mutex = 1;
	else
		return (0);
...
{
	if (! g_queue.tail_mutex)
		g_queue.tail_mutex = 1;
	else
		return (0);
